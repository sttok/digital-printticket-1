                            ...
                            <div class="d-flex">
                                <div class="col-md-12 col-12 mb-2">
                                    <div class="dropdown card-dropdown dropstart" style="float: right;" wire:key="palco-{{$loop->iteration}}">
                                        <button class="btn btn-primary dropstart dropdown-toggle float-right" type="button" id="dropdownMenuButton2" data-bs-toggle="dropdown" aria-expanded="false">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton2" style="">
                                            <li>
                                                @if ($filtrar_por == 0)
                                                    <button class="dropdown-item" type="button" wire:click="venderpalco('{{ $orden['palco'] }}')">
                                                        <i class="fas fa-tag"></i> {{ __('Venta') }}
                                                    </button>
                                                    <button class="dropdown-item" type="button" wire:click="abrirventarapida2('{{ $orden['palco'] }}')" >
                                                        <i class="fas fa-shipping-fast"></i> {{ __('Venta rapida') }}
                                                    </button>
                                                @else
                                                    <button class="dropdown-item" type="button" wire:click="detallepalco('{{ $orden['palco'] }}')" >
